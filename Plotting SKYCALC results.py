...
for i, hdu in enumerate(hdul):
    ...
    if hdu.data is not None:
        ...
        if isinstance(hdu, fits.BinTableHDU):
            ...
            if all(col in data.names for col in ['lam', 'flux']):
                wave = data['lam']  # entire column
                flux_reduced = data['flux']  # entire column

                plt.figure(figsize=(10, 6))
                plt.plot(wave, flux_reduced, label='Flux', color='blue')
